  ...
  <el-container
    ...
    <el-header height="210">
      ...
      <el-alert center show-icon
                title="新增查看证书功能，通过测评即可获得Diego考试系统颁发的专属证书！"
                type="warning">
      </el-alert>
      ...
      <el-table
        ...
        <el-table-column align="center" label="是否通过">
          <template slot-scope="scope">
            ...
            <div v-if="isOrNotPassExam(scope.row) === 1">
            ...
            <span v-if="isOrNotPassExam(scope.row) === 2" style="color: orange">暂未批阅</span>
            <span v-if="isOrNotPassExam(scope.row) === 0" style="color: red">未通过</span>
          </template>
        </el-table-column>
              ...
              <label v-for="(i2,index2) in item.answer"
                ...
                <img style="position: absolute;left:100%;top:50%;transform: translateY(-50%);
                  width: 40px;height: 40px;float: right;cursor: pointer;" title="点击查看大图"
                     v-if="i2.images !== null"
                     v-for="i3 in i2.images" :src="i3" alt="" @click.stop="showBigImg(i3)">
              </label>
            </div>
          </div>
        ...
        this.allExamInfo.forEach(item => {
          if (item.examId === row.examId) {
            if(row.totalScore == null)
              flag = 2
            else if(row.totalScore >= item.passScore)
              flag = 1
          }
        })
        return flag
